...
function findLaunchpadForAgency(agencyName, allLaunchpads) {
...
export default function SpaceTecHub({ apodData, upcomingLaunches, padWeather }) {
  ...
  const handleOpenLaunchpadsForAgency = (agencyName) => {
    const matchedPad = findLaunchpadForAgency(agencyName, allLaunchpads);
    handleOpenAllLaunchpads(matchedPad);
  };
      ...
      <AnimatePresence>
        {showAllAgenciesPage && (
          <AllAgenciesPage 
            ...
            onOpenSatelliteWiki={handleOpenSatelliteWiki}
            onOpenLaunchpads={handleOpenLaunchpadsForAgency}
          />
        )}
      </AnimatePresence>
...
function AllAgenciesPage({ agencies, spaceBackgrounds, onClose, onOpenSatelliteWiki, onOpenLaunchpads, initialAgencyId = null }) {
  const [bgIdx, setBgIdx] = useState(0);
  const [isReturningMain, setIsReturningMain] = useState(false);
